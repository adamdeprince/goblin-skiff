...
goblin-skiff -J user@jump:2222 --jump-port=60001:60999 destination
```

`-J` / `--jump` now routes **the Skiff UDP session**, not just its SSH
...
The updated client, destination server, and a `goblin-skiff-server` on **every
...
(from the client for the first jump), and permit the destination's Skiff port
...
There is no new periodic relay heartbeat. Normal Skiff packets, including the
...
after expiry, start a new session. Destination Skiff timeouts may be shorter.
...
the already-encrypted Skiff datagram in one authenticated envelope per hop.
Each hop strips only its own envelope on the forward path and adds it on
replies. No jump is given the end-to-end Skiff key. A jump can observe traffic
...
full kernel send queue drops a datagram. End-to-end Skiff recovery and the
...
```
MOSH_RELAY_HOPS=N MOSH_CLIENT_CAPS=...,udp-relay-v1 goblin-skiff-server new ...
MOSH RELAY-MTU 1 N
...
```
goblin-skiff-server relay [--fips-crypto] [--port=RANGE] -- TARGET_IP TARGET_PORT
...
ordinary data plaintext is the complete inner datagram. A Skiff keepalive is
not empty at this layer: it still has inner encryption and timestamps.

For a two-hop path the forward wire is `R1(R2(Skiff))`, and replies have the
...
Both Skiff peers reserve all hop overhead against a conservative **1216-byte
outer UDP payload** budget, including mixed IPv4/IPv6 routes. The Skiff inner
...
This is **Skiff-session UDP relaying**, not arbitrary UDP port forwarding.
