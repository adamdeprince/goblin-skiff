...
set -eu
if test "$#" -ne 1; then
    printf 'Usage: sh check-prebuilt-fec.sh /path/to/goblin-moshcp\n' >&2
    exit 2
fi
goblin_moshcp=$1
test -x "$goblin_moshcp"
umask 077
...
goblin_status=0
goblin_result=$("$goblin_moshcp" send --fec=raptorq \
    --socket="$goblin_probe/no-session.sock" "$goblin_probe/no-source" 2>&1) || goblin_status=$?
if test "$goblin_status" -ne 1 || \
   test "$goblin_result" != 'goblin-moshcp: raptorq is not available in this build'; then
