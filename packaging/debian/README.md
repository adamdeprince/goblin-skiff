...
The package is `goblin-mosh`; its commands are installed in `/usr/bin` and
...
The obsolete `adam-mosh` package conflicts with this renamed package.
...
`Containerfile` expects `snapshot.tar.gz` containing one `goblin-mosh/`
...
podman build --build-arg BASE_IMAGE=docker.io/library/debian:12 \
  -f Containerfile -t goblin-mosh-build:bookworm .
mkdir -p artifacts/bookworm
podman run --rm -v "$PWD/artifacts/bookworm:/out" \
  goblin-mosh-build:bookworm bookworm '1.4.0+goblin20260909.1+bookworm1'
```
...
`sh packaging/check-prebuilt-fec.sh /usr/bin/goblin-moshcp` and saves
