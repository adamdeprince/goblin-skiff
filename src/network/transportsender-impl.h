...
void TransportSender<MyState>::send_in_fragments( const std::string& diff, uint64_t new_num )
{
  ...
  if ( advertise_zstd && !local_dictionary_id.empty() ) {
    inst.set_zstd_dict_id( local_dictionary_id );
  }
  const bool allow_zstd_dictionary
    = advertise_zstd && peer_zstd_supported && !local_dictionary_id.empty()
      && ( peer_zstd_dictionary_id == local_dictionary_id );
